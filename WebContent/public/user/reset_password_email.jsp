		  ...
		  <div class="pagebox">
		    ...
		    <div class="main">
		      <div class="container l">
		        <div class="titlebar">
		          <div class="tab tabactive">重置密码</div>
		        </div>
		        <div class="content">
...
<%-- 		          <input name="terminalUser_id" id="terminalUser_id" type="hidden" value="<%=terminalUserVO.getId()%>"></input> --%>
		            ...
		            <input type="button" id="login_btn" class="inputbutton" value="发送邮件"/>
		          </form>
		          <div class="clear">&nbsp;</div>
		        </div>
		      </div>
	...
	function login()
	{
		...
		ajax.remoteCall("bean://terminalUserService:sendMailForChangePassword",
			[formData],
			function(reply)
			{
				...
				else if( reply.result.status=="success" )
				{
					...
					$("<div class=\"datagrid-mask-msg\"></div>").html("发送成功，正在跳转页面。。。").appendTo("body").css({
						display:"block",
...
// 				alert(reply.result.message);
	...
	$(function(){
		...
		// 回车
		$("#email,#verification_code").keypress(function(evt){
			if( evt.keyCode==13 ){
				$("#login_btn").attr("value","正在发送");
				$("#login_btn").removeAttr("disabled");
				login();
			}
		});
