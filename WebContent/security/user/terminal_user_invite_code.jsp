...
<html>
	...
	<body style="visibility:hidden;">
		<form id="big_form"  method="post">
			...
			<table id="invite_code_datagrid" class="easyui-datagrid" title="邀请码管理"
				style="height: 500px; margin: 10px;"
				data-options="
					url: '<%=request.getContextPath()%>/bean/ajax.do?userType=<%=userType%>&bean=inviteCodeService&method=queryInviteCodeForTerminalUser',
					queryParams: {},
					toolbar: '#toolbar',
					rownumbers: true,
					striped: true,
				...
				<thead>
					<tr>
						...
						<th field="code" width="10px" sortable=true>邀请码</th>
<!-- 						<th field="name" width="70px" sortable=true>终端用户名</th> -->
<!-- 						<th field="idCard" width="70px">身份证</th> -->
<!-- 						<th field="email" width="70px">电子邮箱</th> -->
<!-- 						<th field="phone" width="70px">手机号码</th> -->
						<th field="status" formatter="formatStatus" width="10px">状态</th>
						<th field="operate" formatter="terminalUserColumnFormatter" width="10px">操作</th>
					</tr>
				</thead>
			</table>
	...
	function onLoadSuccess()
	{
		...
		$("a.sendEmail_btn").click(function(){
			...
			top.showSingleDialog({
				url: "<%=request.getContextPath()%>/bean/page.do?userType=<%=userType%>&bean=inviteCodeService&method=sendInviteCodeByEmailTerminalUserPage&inviteCodeId="+encodeURIComponent(id),
		...
		$("a.sendPhone_btn").click(function(){
			...
			var id = data.rows[rowIndex].id;
			top.showSingleDialog({
				url: "<%=request.getContextPath()%>/bean/page.do?userType=<%=userType%>&bean=inviteCodeService&method=sendInviteCodeByPhoneTerminalUserPage&inviteCodeId="+encodeURIComponent(id),
				onClose: function(data){
					$('#invite_code_datagrid').datagrid('reload');
				}
			});
...
// 		});
		...
		$("#add_invite_code_btn").click(function(){
			top.showSingleDialog({
				url:"<%=request.getContextPath()%>/bean/page.do?userType=<%=userType%>&bean=inviteCodeService&method=addInviteCodePageByAgent",
				onClose: function(data){
					$('#invite_code_datagrid').datagrid('reload');
				}
			});
		...
		$("#del_invite_code_btn").click(function() {
			...
			top.$.messager.confirm("确认", "确定要删除吗?", function (r) {  
		        if (r) {   
						ajax.remoteCall("bean://inviteCodeService:deleteInviteCodeByIds",
							[ ids ], 
							function(reply) {
