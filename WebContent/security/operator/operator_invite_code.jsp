	...
	<body style="visibility:hidden;">
		<form id="big_form"  method="post">
			...
			<table id="invite_code_datagrid" class="easyui-datagrid" title="邀请码管理"
				...
				data-options="
					url: '<%=request.getContextPath()%>/bean/ajax.do?userType=<%=userType%>&bean=inviteCodeService&method=queryInviteCodeForOperator',
					queryParams: {},
					toolbar: '#toolbar',
					rownumbers: true,
					striped: true,
...
// 		});
		...
		$("#add_invite_code_btn").click(function(){
			top.showSingleDialog({
				url:"<%=request.getContextPath()%>/bean/page.do?userType=<%=userType%>&bean=inviteCodeService&method=addInviteCodePageByOperator",
				onClose: function(data){
					$('#invite_code_datagrid').datagrid('reload');
				}
			});
