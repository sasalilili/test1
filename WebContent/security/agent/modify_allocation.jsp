			...
			<table border="0" style="height: auto; margin: auto;">
				<tr>
					...
					<td style="padding:5px 0 5px 0;">
						...
						<label for="cpu_core_16" style="padding:0 10px 0 0;">
							<input type="radio" id="cpu_core_16"  name="cpu_core" onblur="_cloud_host_modify_allocation_scope_.checkCPUCore()" onclick="_cloud_host_modify_allocation_scope_.updatePrice()" value="16" />
							16核
						</label>
					</td>
				...
				<tr>
					...
					<td style="padding:5px 0 5px 0;">
						<input readonly type="text" name="price" id="price" value="<%=cloudHost.getMonthlyPrice()%>"/>元
					</td>
...
var _cloud_host_modify_allocation_scope_ = new function(){
	...
	self.updatePrice = function(){
		var percentOff = "<%=agent.getPercentOff()%>";
		...
		var bandwidth = $("input[name='bandwidthDIY']").val(); 
		if(region=="1"){ 
			if(bandwidth>100){
				bandwidth = 100;
				$("#bandwidth").val("100");
				top.$.messager.alert('警告','广州带宽最大支持100M','warning');  
			}
		}else if(region=="4"){ 
			if(bandwidth>10){
				bandwidth = 10;
		...
		var newPrice = refreshPrice(region,core,memory,dataDisk,+bandwidth) * (100-percentOff) / 100;
	...
	$(document).ready(function() {
		var core_1 = "<%=cloudHost.getCpuCore()%>";
		var memory_1 = "<%=CapacityUtil.toGBValue(cloudHost.getMemory(),0)%>";
		$("#cpu_core_"+core_1).attr("checked","checked");
		$("#memory_"+memory_1).attr("checked","checked");
		// 提交
