	...
	<body style="visibility:hidden;">
		<form id="big_form"  method="post">
			...
			<table id="invite_code_datagrid" class="easyui-datagrid" title="邀请码管理"
				...
				data-options="
					url: '<%=request.getContextPath()%>/bean/ajax.do?userType=<%=userType%>&bean=inviteCodeService&method=queryInviteCodeForAgent',
					queryParams: {},
					toolbar: '#toolbar',
					rownumbers: true,
					striped: true,
	...
	function onLoadSuccess()
	{
		...
		$("a.sendEmail_btn").click(function(){
			...
			top.showSingleDialog({
				url: "<%=request.getContextPath()%>/bean/page.do?userType=<%=userType%>&bean=inviteCodeService&method=sendInviteCodeByEmailPage&inviteCodeId="+encodeURIComponent(id),
		...
		$("a.sendPhone_btn").click(function(){
			...
			var id = data.rows[rowIndex].id;
			top.showSingleDialog({
				url: "<%=request.getContextPath()%>/bean/page.do?userType=<%=userType%>&bean=inviteCodeService&method=sendInviteCodeByPhonePage&inviteCodeId="+encodeURIComponent(id),
				onClose: function(data){
					$('#invite_code_datagrid').datagrid('reload');
				}
			});
